...
const GameRuntime = (() => {
	...
	const renderHUD = () => {
		...
		if (activeDistrict !== 'COMBAT' && activeDistrict !== 'TITLE' && activeDistrict !== 'GAME_OVER') {
	...
	const updateOverworldSubsystems = (dt) => {
		if (activeDistrict === 'OVERWORLD' && typeof EmberlightOverworld !== 'undefined' && typeof EmberlightOverworld.update === 'function') {
			const diag = typeof EmberlightOverworld.getDiagnostics === 'function' ? EmberlightOverworld.getDiagnostics() : null;
			if (diag?.lifecycleState === 'READY' || diag?.lifecycleState === 'RUNNING') {
