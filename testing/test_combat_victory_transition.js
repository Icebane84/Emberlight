	...
	const el = {
		...
		classList: {
			...
			toggle: (cls, force) => {
				...
				} else if (classSet.has(cls)) classSet.delete(cls);
...
const sandbox = {
	...
	Map,
	JSON,
	structuredClone: (obj) => (typeof structuredClone === 'function' ? structuredClone(obj) : structuredClone(obj))
};
...
for (const scriptFile of scripts) {
	...
	vm.runInContext(code, context, { filename: scriptFile }); // NOSONAR: Test harness requires loading vanilla JS modules into headless DOM VM context
}
