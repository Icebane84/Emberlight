...
const EmberlightDynamicLights = (() => {
	...
	const membrane = (typeof window !== 'undefined' && window._DynamicLightsInternal)
	...
	const {
		...
		initDustMotes = () => {},
		calculateScreenPlayerCoords = (px, py, tw, th, ox, oy) => ({ pScreenX: ox + px * tw, pScreenY: oy + py * th }),
	...
	function render(time = 0) {
		...
		const { pScreenX, pScreenY } = calculateScreenPlayerCoords(playerX, playerY, tileW, tileH, offsetX, offsetY, canvas);

		const isSubterranean = dungeonDepth > 0 || activeZoneType === 'CRYPT';
		const ambientDarkness = isSubterranean ? 0.94 : 0.0;

		const emitters = assembleEmitters(pScreenX, pScreenY, flicker, offsetX, offsetY, isSubterranean, cachedMap, tileW, tileH, flickerTime, transientLights);

		renderDarknessAndShadows(ctx, w, h, ambientDarkness, emitters, offsetX, offsetY, cachedMap, tileW, tileH, castShadowFromOccluder);
		renderRadiantAndBevelHighlights(ctx, emitters, offsetX, offsetY, cachedMap, tileW, tileH, drawBeveledWallEdges);

		if (isSubterranean) {
			renderDustMotes(ctx, w, h, pScreenX, pScreenY, dustMotes, initDustMotes, moteRand);
		}
	...
	return {
				...
				unsubs.push(
					eventBus.subscribe('overworld:step', (payload) => {
						...
						if (pos) {
							targetPlayerX = pos.x;
							targetPlayerY = pos.y;
							activeZoneType = evaluateZone(pos.x, pos.y, dungeonDepth, cachedMap);
						}
					}),
					eventBus.subscribe('overworld:map_loaded', ({ map, depth, pos }) => {
						...
						activeZoneType = evaluateZone(targetPlayerX, targetPlayerY, dungeonDepth, cachedMap);
					}),
					eventBus.subscribe('combat:sfx', ({ sfx }) => {
						const sx = targetPlayerX * tileW + tileW * 0.5;
						const sy = targetPlayerY * tileH + tileH * 0.5;
						if (sfx === 'SPELL_BOLT') {
							spawnTransientLight(transientLights, MAX_TRANSIENTS, sx, sy, 180, '56, 189, 248', 0.45, 1.4);
						} else if (sfx === 'ATTACK_HIT') {
							spawnTransientLight(transientLights, MAX_TRANSIENTS, sx, sy, 120, '255, 157, 77', 0.25, 1.1);
						} else if (sfx === 'HEAL') {
							spawnTransientLight(transientLights, MAX_TRANSIENTS, sx, sy, 160, '74, 222, 128', 0.6, 1.2);
						}
					}),
					eventBus.subscribe('overworld:transmute_request', ({ type, playerPos }) => {
						...
						if (type === 'SCORCH') {
							spawnTransientLight(transientLights, MAX_TRANSIENTS, px, py, 220, '239, 68, 68', 0.6, 1.8);
						} else if (type === 'CONSECRATE') {
							spawnTransientLight(transientLights, MAX_TRANSIENTS, px, py, 240, '250, 204, 21', 0.8, 1.5);
						}
					})
				);
			}
		...
		spawnFlare(x, y, radius, colorRgb, duration) {
			...
			spawnTransientLight(transientLights, MAX_TRANSIENTS, sx, sy, radius, colorRgb, duration);
		},
