...
7. **Sentinel Governance (MVP-001):** Autonomous boot gatekeeper executing a 21-pass anti-entropy verification battery asserting 136/136 checks (100% genuine, anti-theater compliance) including persistence compression budgets ($<2.5\text{KB}$), EventBus unbind idempotency, peripheral driver lifecycle teardown, dual-perspective immersion scaling, boss phase enrage shifts, discrete DOT tick formulas, ailment cleansing, procedural $64\times64$ battler sprite generation, biome backdrop depth horizon rendering, zero-allocation typed array particle clamping, district transition/chest collision invariance, dynamic gear stat projection deltas, quest hierarchy validation, in-situ field pouch vital targeting, the complete 10-point VSRP-001 Constitutional Compliance battery (`AC-01` to `AC-10`), Pass 18 Tactical Displacement & Row Invariance, Pass 19 Deep Analysis Workstation & Tactical Terminal Canvas, Pass 20 4-Quadrant War Table Skeleton & Projection Battery, and Pass 21 4-Quadrant Combat Station Integration Battery before the title screen mounts.
...
36. **4-Quadrant War Table Skeleton & Matrix Invariance Protocol (ARCH-SPEC-WAR-TABLE-SKELETON-001 / Pass 20):**
    - _DOM Topology Invariance:_ `#war-table-matrix` remains permanently mounted in the DOM across both Exploration and Combat modes with all 4 quadrant panes (`#pane-cartography`, `#pane-sensor`, `#pane-scanner`, `#pane-readiness`), completely deprecating `#combat-theater` destruction and reconstruction cycles.
    - _Layer Swapping via CSS:_ Toggling combat state applies `.combat-active-matrix` to `#war-table-matrix`, seamlessly switching active layers within each quadrant (Q1: `#combat-spatial-view`, Q2: `#combat-arena-view`, Q3: `#combat-oracle-view`, Q4: `#combat-command-view`) without tearing down WebGL or 2D canvas rendering contexts.
    - _PMIP-001 Enveloping & Re-Entrant FIFO Dispatch:_ All cross-district/cross-quadrant event bus traffic is encapsulated in frozen PMIP-001 envelopes (`EventBus.createEnvelope()`), while `EventBus.publish()` employs a FIFO queue to prevent synchronous recursion and call-stack overflow during nested event publishing.
    - _Tri-Partite Projection DTOs:_ `combat.js` constructs and deep-freezes 4 discrete projection DTOs (`q1Spatial`, `q2Clash`, `q3Oracle`, `q4Deck`) on every render tick and transmits them to presentation drivers via `renderWarTable(projection, dispatch)`, ensuring zero direct state mutation and zero lateral cross-quadrant leakage.

37. **4-Quadrant Combat Station Full Integration Protocol (ARCH-SPEC-COMBAT-STATION-002 / Pass 21):**
    - _Q1 Tactical Flank & Kinematics Canvas:_ Dedicated 2D HTML5 canvas (`#combat-spatial-canvas`) rendering an $8\times6$ tactical battle grid, front/back row unit tokens, hazard dungeon walls at columns 0 and 7, dynamic displacement trajectory arrows (`KNOCKBACK` / `PULL`) with wall impact telemetry (`💥 SLAM!`), and coordinate click/hover targeting.
    - _Q2 First-Person 3D Clash Theater:_ Eye-level composite arena (`#combat-backdrop-canvas`) featuring Z-depth scaled paper-doll battlers ($Z=0.75$ to $1.20$), dynamic spell illumination bursts (`dynamic_lights.js`), attack lunges, and direct click target selection on 3D enemy models.
    - _Q3 Three-Tier Threat Oracle:_ Structured 3-tier console with 1) Interactive 12-slot CTB forward turn ribbon with dynamic post-action slide preview on skill hover; 2) Hostile intent beacons (`🎯 Shade Wolf ➔ Wren ⚔️ [BASIC STRIKE]`); 3) Elemental affinity matrix badges (`WEAK: FIRE (1.5x)`, `RESIST: DARK (0.5x)`) and compact scrolling engagement feed (`#log-box`).
    - _Q4 Hero Chassis Command Deck:_ 4 physical hero chassis cards (`#combat-hero-chassis-grid`) with live dual HP/MP progress bars, front/back row tags, and amber glowing active-turn elevation. Right-clicking the active hero card opens the 4-leaf quick action radial menu (`#combat-hero-radial`: North=SKILLS, East=POUCH, South=GUARD, West=ATTACK), integrated with dual-input targeting across Q1 tokens and Q2 models.
