...
const EmberlightManifest = (() => {
	...
	internal = internal || {};
	const { DefaultSettings, SessionConfig, ExplorationConfig } =
		internal.Config || {};
	const { Curves, PartyRoster, Phenotypes, Enemies, Encounters } =
		internal.Actors || {};
	const { Items } = internal.Items || {};
	const { AetherEssences, AetherNodes, SkillTrees } =
		internal.Progression || {};
	const { TileLegend, OverworldMap, TownMaps } = internal.World || {};
	const { Ailments, Quests, WorldMutations, Shops, Dialogues } =
		internal.Narrative || {};
	const { calculateGearStats, computeCharacterStats, getResolvedMap } =
