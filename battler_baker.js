/* =========================================================================
 * EMBERLIGHT BATTLER BAKER — V4.0.0
 * =========================================================================
 * VSRP-001-BATTLER-BAKER-V4
 *
 * Production-oriented procedural 64x64 battler asset compiler.
 *
 * ARTISTIC UPGRADE OVER V2/V3
 * ----------------------------
 * - Designed silhouettes instead of rectangle-only anatomy.
 * - Multi-tone material palettes: shadow / mid / light / specular.
 * - Layered occlusion: rear limbs -> torso -> front limbs -> equipment.
 * - Anatomical landmarks: neck, jaw, shoulders, elbows, knees, hands.
 * - Facial construction and readable eye/visor treatment.
 * - Cloth folds, plate segmentation, leather straps, chain highlights.
 * - Weapon guards, grips, blades, heads, runes and emissive cores.
 * - Enemy-specific silhouettes instead of visual aliases wherever possible.
 * - 4x supersampling + fractional highlight geometry.
 * - Deterministic rendering and formal canonical/fallback diagnostics.
 *
 * QUALITY CONTRACT
 * ----------------
 * A successful render is NOT sufficient evidence of correct identity.
 * The resolver reports whether the requested asset was:
 *   INTENDED            exact canonical asset
 *   ALIAS               explicit canonical alias
 *   FALLBACK            documented intentional fallback
 *   SEMANTIC_FALLBACK   semantic/default resolution
 *   ERROR_FALLBACK      safe asset returned after render failure
 *   UNRESOLVED          no asset could be resolved
 *
 * Public API remains compatible with VSRP-001:
 * configure, init, reset, update, render, getState, getDiagnostics,
 * getModuleInfo, destroy
 *
 * Plus legacy/convenience:
 * get, bakeAll, getDiagnosticReport, validateCanonicalAssets
 * ========================================================================= */
...
const EmberlightBattlerBaker = (() => {
  ...
  function rr(ctx, x, y, w, h, r, fill, alpha = 1) {
    ctx.save();
    ctx.globalAlpha = alpha;
    ctx.fillStyle = fill;
  ...
  function spec(ctx, x, y, w, h, color = P.steel4, alpha = 0.75) {
    // Fractional geometry intentionally survives until final downsample.
    rect(ctx, x + 0.25, y + 0.25, w, h, color, alpha);
  }

  /* =======================================================================
   * HEROES — designed silhouettes, anatomy, materials
   * ======================================================================= */
  ...
  function drawHero(ctx) {
    ...
    // Legs — readable knee and boot construction.
    poly(
      ctx,
      [
        [22, 46],
  ...
  function drawWarrior(ctx) {
    ...
    // Great axe — strong diagonal silhouette.
    line(
      ctx,
      [
        [47, 48],
  ...
  function armor(ctx, id, phenotype) {
    ...
    if (a.includes("CHAIN") || a.includes("MAIL")) {
      // Subtle chain pattern without destroying silhouette.
    ...
    if (a.includes("PLATE") || a.includes("IRON") || a.includes("STEEL")) {
      line(
        ctx,
        [
          [25, 29],
    ...
    if (a.includes("LEATHER") || a.includes("VEST") || a.includes("STUDDED")) {
      line(
        ctx,
        [
          [24, 31],
  ...
  function weapon(ctx, id, phenotype) {
  ...
  /* =======================================================================
   * ENEMIES — each canonical enemy receives a deliberately distinct visual
   * identity. No "Iron Brute = skeleton" art substitution.
   * ======================================================================= */
  ...
  function caveSpider(ctx) {
    ...
    // Eight legs with segmented joints.
    const legs = [
      [
        [24, 35],
        [15, 29],
  ...
  function composeHeroToCanvas(workCtx, phenotype, weaponId, armorId) {
    // Base art is already fully authored. Equipment overlays can add identity
    // details for callers that request non-default gear.
  ...
  function parseSpec(spec, options) {
    if (spec && typeof spec === "object") {
      return {
        id: spec.id || spec.phenotype || "HERO",

        phenotype: spec.phenotype || spec.id || "HERO",
