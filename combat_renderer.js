...
const EmberlightCombatRenderer = (() => {
	...
	/**
	 * Resolves default ally slot index based on hover, active turn, or first living unit.
	 * @returns {number} Resolved ally slot index.
	 */
	function resolveContextualHeroIndex() {
		if (
			ephemeralHover?.type === "HERO" &&
			typeof ephemeralHover.index === "number"
		) {
		...
		const firstLiving = party.findIndex((p) => p.alive);
		return firstLiving !== -1 ? firstLiving : 0;
	}

	/**
	 * Resolves default enemy slot index based on hover, focus-fire target, or first living unit.
	 * @returns {number} Resolved enemy slot index.
	 */
	function resolveContextualEnemyIndex() {
		if (
			ephemeralHover?.type === "ENEMY" &&
			typeof ephemeralHover.index === "number"
		) {
	...
	function emit(action) {
		...
		if (
			...
			actObj.targetIndex = actObj.isAlly
				? resolveContextualHeroIndex()
				: resolveContextualEnemyIndex();
		}
	...
	function getNormalizedCanvasCoords(canvas, event) {
		...
		return {
			...
			normX: Math.max(
				0,
				Math.min(1, cssX / Math.max(1, rect.width)),
			),
			normY: Math.max(
				0,
				Math.min(1, cssY / Math.max(1, rect.height)),
			),
		};
	}
	...
	function scheduleSynchronizedRedraw() {
		...
		if (typeof requestAnimationFrame !== "undefined") {
			requestAnimationFrame(() => {
				...
				if (lastCombatState) {
					if (lastQ1Spatial)
						renderQ1SpatialCanvas(lastQ1Spatial, lastCombatState);
					if (lastQ2Clash) renderQ2ClashCanvas(lastQ2Clash, lastCombatState);
					if (lastQ3Oracle)
						renderQ3ThreatOracleDeck(lastQ3Oracle, lastCombatState);
	...
	function setEphemeralHover(hoverTarget) {
		const newKey = hoverTarget
			? `${hoverTarget.type}_${hoverTarget.id}_${hoverTarget.index}`
			: "";
			...
			if (activeFlickDirection !== dir) {
				...
				radial.querySelectorAll(".radial-leaf-btn").forEach((btn) => {
					btn.classList.toggle(
						"active-flick",
						btn.classList.contains(dir.toLowerCase()),
					);
				});
	...
	function syncDOMHighlighting() {
		...
		// Q3 CTB Slots
		document
			.querySelectorAll("#combat-ctb-ribbon-bar .turn-slot")
			.forEach((slot) => {
				const slotId = /** @type {HTMLElement} */ (slot).dataset.entityId;
				if (hId && slotId === hId) {
					slot.classList.add("hovered-slot", "the-flare-q3");
				} else {
					slot.classList.remove("hovered-slot", "the-flare-q3");
				}
			});
		...
		document
			.querySelectorAll(".hero-pedestal-stage, .hero-chassis-card")
			.forEach((el) => {
	...
	function resolveAffinity(attackElement, target) {
		if (!target || !attackElement || attackElement === "NONE")
			return { label: "NEUTRAL", multiplier: 1.0 };
		const elem = String(attackElement).toUpperCase();
		const weaknesses = (target.weaknesses || []).map((w) =>
			String(w).toUpperCase(),
		);
		const resistances = (target.resistances || []).map((r) =>
			String(r).toUpperCase(),
		);
		const immunities = (target.immunities || []).map((i) =>
	...
	function renderClashHeader(state) {
		...
		if (state?.pendingSkill) {
			...
			clashLabelEl.textContent = (
				state.pendingSkill.label ||
				state.pendingSkill.name ||
				"SKILL"
			).toUpperCase();
	...
	function renderSkillTelemetry(
		...
		const rawBase = Math.max(
			1,
			(activeChar.atk || 10) * powerMult * 1.5 - (targetEnemy.def || 5),
		);
	...
	function renderBasicTelemetry(
		...
		const rawBase = Math.max(
			1,
			(activeChar.atk || 10) * 2 - (targetEnemy.def || 5),
		);
		const estMin = Math.max(1, rawBase - 2);
		const estMax = Math.max(1, rawBase + 3);
		if (telemetryAffinity) {
			telemetryAffinity.innerHTML = "AFFINITY: PHYSICAL (1.0x)";
		}
		if (telemetryDmg) {
			telemetryDmg.textContent = `EST DMG: ${estMin} ~ ${estMax}`;
		}
		if (telemetryHit) {
			telemetryHit.textContent = "HIT: 95%";
		}
	}
	...
	/**
	 * Renders 4 glassmorphic telemetry chips into #combat-telemetry-chips in Q4.
	 * @param {Battler|null} activeChar Active hero battler.
	 * @param {Battler|null} targetEnemy Target enemy battler.
	 * @param {CombatState} state Combat state snapshot.
	 */
	function renderTelemetryChips(activeChar, targetEnemy, state) {
		const container = getElement("combat-telemetry-chips");
		if (!container) return;
		...
		const foeDef = targetEnemy?.def || 5;
		const foeAgi = targetEnemy?.agi || 8;
		const heroAgi = activeChar?.agi || 10;
		...
		if (targetEnemy?.alive) {
			...
			const powerMult =
				...
				activeSkill?.mult ||
				(activeSkill?.power ? activeSkill.power / 10 : 1.0);
			const rawBase = Math.max(1, heroAtk * powerMult * 1.5 - foeDef);
			const finalDmg = Math.max(1, Math.round(rawBase * affinity.multiplier));
			const estMin = Math.max(1, Math.round(finalDmg * 0.85));
			const estMax = Math.max(1, Math.round(finalDmg * 1.15));
			const hitChance = Math.min(
				...
				Math.max(
					50,
					(activeSkill?.accuracy || 95) + (heroAgi - foeAgi) * 2,
				),
			);
			const delayCost =
				activeSkill?.delayCost || (activeSkill ? 1100 : 1000);
			const estDelay = Math.round(delayCost / Math.max(1, heroAgi));

			let affCls = "";
			let affText = `${activeElement} (1.0x)`;
			if (affinity.label === "WEAK") {
				affCls = "weak";
				affText = `⚡ WEAK (1.5x)`;
			} else if (affinity.label === "RESIST") {
				affCls = "resist";
				affText = `🛡️ RESIST (0.5x)`;
			} else if (affinity.label === "IMMUNE") {
				affCls = "resist";
				affText = `🚫 IMMUNE (0.0x)`;
			}
			...
			container.innerHTML = `
				<div class="telemetry-chip">
					<span class="chip-label">💥 EST. DMG</span>
					<span class="chip-val highlight">${estMin} ~ ${estMax}</span>
				</div>
				<div class="telemetry-chip">
					<span class="chip-label">🎯 ACCURACY</span>
					<span class="chip-val">${hitChance}%</span>
				</div>
				<div class="telemetry-chip">
					<span class="chip-label">⏳ TURN DELAY</span>
					<span class="chip-val">+${estDelay}t</span>
				</div>
				<div class="telemetry-chip">
					<span class="chip-label">⚡ AFFINITY</span>
					<span class="chip-val ${affCls}">${affText}</span>
				</div>
			`;
		} else {
			container.innerHTML = `
				<div class="telemetry-chip">
					<span class="chip-label">💥 EST. DMG</span>
					<span class="chip-val highlight">--</span>
				</div>
				<div class="telemetry-chip">
					<span class="chip-label">🎯 ACCURACY</span>
				...
				<div class="telemetry-chip">
					<span class="chip-label">⚡ AFFINITY</span>
					<span class="chip-val">READY</span>
				</div>
			`;
		}
	}
	...
	function resolveEnemyBattlerUrl(enemy, isBossEnraged, heatPulse) {
		if (
			...
			typeof EmberlightBattlerBaker.get === "function"
		) {
	...
	function resolveBossDetails(e) {
		const isBossUnit = Boolean(
			e.isBoss ||
			(e.key && String(e.key).toUpperCase().includes("BOSS")) ||
			(e.key && String(e.key).toUpperCase().includes("MALAKOR")),
		);
		const isBossEnraged = Boolean(e.isBoss && e.phaseTwoActive);
		const heatPulse = isBossEnraged
			? 1.0 + Math.sin(Date.now() * 0.008) * 0.25
			: 1.0;
	...
	function formatAilments(e) {
		...
		const ailmentClasses = ailments
			.map((a) => `has-ailment-${a.id.toLowerCase()}`)
			.join(" ");
		const ailmentTags = ailments
	...
	function renderEnemyCard({ e, idx }, activeElement) {
		...
		const rowClass =
			e.row === "FRONT" || e.row === "BOTH" || !e.row
				? "row-front"
				: "row-back";
		const rowTag =
			e.row === "BACK"
				? '<div class="backline-tag">BACKLINE (+15% EVA)</div>'
				: '<div class="frontline-tag">FRONTLINE</div>';
		const stanceClass = isExhausted ? "exhausted" : "";
		const exhaustedBadge = isExhausted
			? '<div class="exhausted-badge">⚠️ TIRED (≤30%)</div>'
			: "";
	...
	function renderEnemyWing(container, state, activeChar) {
		...
		const frontEnemies = enemies
			.map((e, idx) => ({ e, idx }))
			.filter(({ e }) => e.row === "FRONT" || e.row === "BOTH" || !e.row);
	...
	function renderPartyCard({ c, idx }, currentTurnEntity) {
		...
		const hpPct =
			maxHp > 0
				? Math.max(0, Math.min(100, Math.round((curHp / maxHp) * 100)))
				: 0;
		const mpPct =
			maxMp > 0
				? Math.max(0, Math.min(100, Math.round((curMp / maxMp) * 100)))
				: 0;
		const isExhausted = c.alive && curHp / maxHp <= 0.3;
		const ailmentList = (c.ailments || []).map((a) => a.id).join(",");
		const ailmentClasses = (c.ailments || [])
			.map((a) => `has-ailment-${a.id.toLowerCase()}`)
			.join(" ");
		...
		const rowClass =
			c.row === "FRONT" || c.row === "BOTH" || !c.row
				? "row-front"
				: "row-back";
		const rowTag =
			c.row === "BACK"
				? '<div class="backline-tag">ARCANIST BACK</div>'
				: '<div class="frontline-tag">VANGUARD FRONT</div>';
		const stanceClass = isExhausted ? "exhausted" : "";
		const exhaustedBadge = isExhausted
			? '<div class="exhausted-badge">⚠️ TIRED (≤30%)</div>'
			: "";
    ...
    `;
		...
		if (
			state.phase === "TARGETING_ALLY" ||
			state.pendingSkill?.targetType === "ally" ||
			state.pendingItem
		) {
	...
	function buildTargetTelemetryHtml(
		...
		const hitChance = Math.min(
			100,
			Math.max(60, 95 + ((activeChar?.agi || 10) - (targetEnemy.agi || 8)) * 2),
		);
			...
			<div class="telemetry-ribbon-card">
				...
				<div class="telemetry-ribbon-metrics">
					...
					<div class="telemetry-metric-item">
						...
						<span class="metric-val ${affDesc.cls}">${affDesc.label}</span>
					</div>
				</div>
	...
	function renderSubdeckAttack(subDeck, state, activeChar = null) {
		...
		if (enemies.length === 0) {
			subDeck.innerHTML =
				'<div class="subdeck-empty">All hostile targets eliminated.</div>';
			return;
		}

		const activeSkill = state.pendingSkill || ephemeralPreviewSkill;
		const activeElement = getActiveSkillElement(activeSkill);
		const targetIdx = resolveContextualEnemyIndex();
	...
	function collectHeroSkills(activeChar) {
		...
		if (manifest.AetherNodes) {
			unlockedIds.forEach((nodeId) => {
				...
				if (
					node?.type === "active" &&
					!activeSkills.some((s) => s.id === node.id)
				) {
      ...
      <div class="skills-deck-grid">
        ${activeSkills
				.map((node) => {
					...
					const iconUrl =
						typeof EmberlightSkillIcons !== "undefined"
							? EmberlightSkillIcons.get(node.id)
							: null;
					const elemBadge = node.element
						? `<span class="skill-elem-tag ${node.element.toLowerCase()}">${node.element}</span>`
						: "";
    ...
    `;
		...
		subDeck
			.querySelectorAll(".skill-deck-card:not(.disabled)")
			.forEach((card) => {
				/** @type {HTMLElement} */
				...
				if (skillNode) {
					htmlCard.addEventListener("mouseenter", () => {
						const delay =
							(skillNode.delayCost || 1100) /
							Math.max(1, activeChar?.agi || 10);
						if (
							typeof EmberlightThreatOracle !== "undefined" &&
							typeof EmberlightThreatOracle.forecastActionTimeline ===
							"function"
						) {
							const forecast = EmberlightThreatOracle.forecastActionTimeline(
								state.party || [],
								state.enemies || [],
								activeChar?.id,
								delay,
							...
							EmberlightThreatOracle.render(
								"combat-ctb-ribbon-bar",
								{
									...
									bossIntent:
										(state.enemies || []).find((e) => e.isBoss && e.alive) ||
										null,
								},
								emit,
							);
	...
	function renderSubdeckPouch(subDeck, state) {
      ...
      `;
			...
			if (cancelBtn) {
				cancelBtn.addEventListener("click", () =>
					emit({ type: "CANCEL_ITEM" }),
				);
			}

			subDeck
				.querySelectorAll("[data-item-target-idx]:not([disabled])")
				.forEach((btn) => {
					/** @type {HTMLElement} */
					const htmlBtn = /** @type {HTMLElement} */ (btn);
		...
		const items = [
			{
				id: "POTION",
				name: "Health Potion",
				effect: "Restores 30 HP to an ally",
				count: inv.POTION || 0,
			},
			{
				id: "ETHER",
				name: "Ether Flask",
				effect: "Restores 15 MP to an ally",
				count: inv.ETHER || 0,
			},
			{
				id: "PHOENIX_EMBER",
				name: "Phoenix Ember",
				effect: "Revives fallen ally (50% HP)",
				count: inv.PHOENIX_EMBER || 0,
			},
		];
      ...
      <div class="pouch-deck-grid">
        ${items
				.map((item) => {
					...
					const icons = win.EmberlightIcons || win.EmberlightPartyIcons;
					const iconUrl =
						icons && typeof icons.get === "function"
							? icons.get(item.id)
							: null;
					return `
    ...
    `;
		...
		subDeck
			.querySelectorAll(".pouch-deck-card:not(.disabled)")
			.forEach((card) => {
				/** @type {HTMLElement} */
				const htmlCard = /** @type {HTMLElement} */ (card);
	...
	function renderVictoryView(subDeck, title, tag, ribbon, state) {
      ...
      <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; padding:14px 16px; text-align:center; background:rgba(12,18,32,0.95); border:1.5px solid var(--ember); border-radius:6px; box-shadow:0 0 20px rgba(255,157,77,0.35);">
        ...
        <button type="button" class="cmd-btn action" id="victory-continue-btn" style="padding:8px 24px; font-size:11px; font-weight:700; letter-spacing:0.5px; cursor:pointer;">▶ CONTINUE EXPEDITION [SPACE]</button>
      </div>
    `;
	...
	function renderDefeatView(subDeck, title, tag, ribbon, _state) {
		...
		ribbon.innerHTML = "";
		subDeck.innerHTML = `
      <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; padding:14px 16px; text-align:center; background:rgba(20,8,12,0.95); border:1.5px solid var(--danger); border-radius:6px; box-shadow:0 0 20px rgba(239,68,68,0.35);">
        <div style="font-size:15px; font-weight:800; color:var(--danger); margin-bottom:4px; letter-spacing:1px;">💀 PARTY HAS FALLEN 💀</div>
        <div style="font-size:11.5px; color:#cbd5e1; margin-bottom:12px;">The squad collapsed in combat.</div>
        <button type="button" class="cmd-btn" id="defeat-continue-btn" style="padding:8px 24px; font-size:11px; font-weight:700; border-color:var(--danger); color:var(--danger); cursor:pointer;">▲ ADVANCE TO HARBOR [SPACE]</button>
      </div>
    `;
	...
	/**
	 * Renders primary command deck tabs ribbon.
	...
	/**
	 * Dispatches subdeck view rendering based on selected command tab.
	 * @param {HTMLElement} subDeck
	 * @param {string} selectedTab
	 * @param {CombatState} state
	 * @param {Battler|null} activeChar
	 * @param {HTMLElement|null} fleeBtn
	 */
	function renderSubdeckTabs(subDeck, selectedTab, state, activeChar, fleeBtn) {
		subDeck.innerHTML = "";
	...
	function renderCommandHub(state, activeChar) {
		...
		if (title)
			title.textContent = `🎯 ${activeChar ? (activeChar.name || "HERO").toUpperCase() : "PARTY"} COMMAND HUB`;
		...
		renderCommandDeckRibbon(ribbon, selectedTab);
		renderSubdeckTabs(subDeck, selectedTab, state, activeChar, fleeBtn);
	}

	/**
	 * Resolves class glyph emoji for ally phenotype.
	 * @param {string} [phenotype] Hero phenotype.
	 * @returns {string} Glyph emoji.
	 */
	function resolveAllyTokenGlyph(phenotype) {
		switch (phenotype) {
			case "MAGE":
				return "✨";
			case "HEALER":
	...
	/**
	 * Resolves threat glyph emoji for hostile enemy.
	 * @param {Battler} enemy Hostile battler node.
	 * @returns {string} Glyph emoji.
	 */
	...
	/**
	 * Draws semi-transparent sector zone backgrounds on Q1 canvas.
	 * @param {CanvasRenderingContext2D} ctx 2D rendering context.
	 * @param {number} cellW Grid cell width.
	 * @param {number} h Total canvas height.
	 */
	...
	/**
	 * Draws tactical zone header labels on Q1 canvas.
	 * @param {CanvasRenderingContext2D} ctx 2D context.
	 * @param {number} cellW Cell width.
	 */
	function drawQ1ZoneHeaders(ctx, cellW) {
	...
	function drawQ1ThreatVectors(
		...
		if (activeHero && targetedEnemy?.alive && activeHero.alive) {
			...
			ctx.beginPath();
			ctx.moveTo(
				toCx - Math.cos(angle - 0.5) * arrowLen,
				toCy - Math.sin(angle - 0.5) * arrowLen,
			);
			ctx.lineTo(toCx, toCy);
			ctx.lineTo(
				toCx - Math.cos(angle + 0.5) * arrowLen,
	...
	/**
	 * Draws physical displacement trajectory vectors and wall-slam markers.
	 * @param {CanvasRenderingContext2D} ctx 2D context.
	 * @param {Array<any>} displacementVectors Displacement vector list.
	 * @param {number} cellW Cell width.
	 * @param {number} cellH Cell height.
	 */
	function drawQ1DisplacementVectors(ctx, displacementVectors, cellW, cellH) {
		displacementVectors.forEach((vec) => {
			...
			if (ctx.beginPath && ctx.stroke) {
				...
				ctx.beginPath();
				ctx.moveTo(
					toCx - Math.cos(angle - 0.5) * 8,
					toCy - Math.sin(angle - 0.5) * 8,
				);
				ctx.lineTo(toCx, toCy);
				ctx.lineTo(
					toCx - Math.cos(angle + 0.5) * 8,
	...
	/**
	 * Draws a diamond ally token on Q1 spatial radar.
	 * @param {CanvasRenderingContext2D} ctx 2D context.
	 * @param {any} ally Party member node.
	 * @param {number} cellW Cell width.
	 * @param {number} cellH Cell height.
	 */
	...
	/**
	 * Resolves stroke, fill, and text colors for Q1 enemy token.
	 * @param {any} enemy Hostile battler node.
	 * @param {boolean} isHovered
	 * @returns {{ enemyFill: string, enemyStroke: string, labelColor: string }}
	 */
	function resolveEnemyTokenStyles(enemy, isHovered) {
		...
		const stroke = isHovered ? "#ff9d4d" : enemyStroke;
		let labelColor = "#64748b";
		if (enemy.alive) {
			labelColor = enemy.isBoss ? "#fca5a5" : "#e9d5ff";
		}
		return { enemyFill, enemyStroke: stroke, labelColor };
	}

	/**
	 * Traces a hexagonal path on context.
	 * @param {CanvasRenderingContext2D} ctx
	 * @param {number} cx
	 * @param {number} cy
	 * @param {number} size
	 */
	function traceHexagon(ctx, cx, cy, size) {
		ctx.beginPath();
		for (let a = 0; a < 6; a++) {
	...
	function renderQ1SpatialCanvas(q1Spatial, _state) {
		...
		const canvas = /** @type {HTMLCanvasElement|null} */ (
			getElement("combat-spatial-canvas")
		);
		...
		drawQ1HazardWalls(ctx, q1Spatial?.hazardTiles || [], cellW, cellH);
		drawQ1ThreatVectors(
			ctx,
			q1Spatial?.threatVectors || [],
			q1Spatial?.allies || [],
			q1Spatial?.enemies || [],
			cellW,
			cellH,
		...
		// Displacement Trajectory Vectors
		...
		if (ephemeralPreviewSkill?.displacement) {
			...
			displacementVectors.push(...(q1Spatial?.activeVectors || []));
		}
		...
		// Bind Pointer Events on Q1 Canvas Idempotently
		if (!q1EventsBound) {
			q1EventsBound = true;
			canvas.addEventListener("pointermove", (ev) => {
				...
				const hitAllyIdx = allies.findIndex(
					(a) => a.gridX === gx && a.gridY === gy && a.alive,
				);
				if (hitAllyIdx !== -1) {
	...
	/**
	 * Draws battler contact shadow on floor.
	 * @param {CanvasRenderingContext2D} ctx
	 * @param {number} x
	 * @param {number} y
	 * @param {number} spriteSize
	 */
	function drawBattlerContactShadow(ctx, x, y, spriteSize) {
	...
	/**
	 * Draws boss aura / enrage pulse glow.
	 * @param {CanvasRenderingContext2D} ctx
	 * @param {number} x
	 * @param {number} y
	 * @param {number} spriteSize
	 * @param {any} enemy
	 */
	function drawBossAuraRing(ctx, x, y, spriteSize, enemy) {
		if (ctx.beginPath && ctx.arc && ctx.fill) {
			...
			ctx.fillStyle = enemy.phaseTwoActive
				? "rgba(239, 68, 68, 0.25)"
				: "rgba(168, 85, 247, 0.2)";
			ctx.fill();
			ctx.restore();
	...
	/**
	 * Draws fallback sprite placeholder when battler image is not available.
	 * @param {CanvasRenderingContext2D} ctx
	 * @param {any} enemy
	 * @param {number} x
	 * @param {number} y
	 ...
	 */
	function drawBattlerFallbackPlaceholder(
		ctx,
		enemy,
		x,
		y,
	...
	/**
	 * Draws high-tech targeting bracket on hover or focus target.
	 * @param {CanvasRenderingContext2D} ctx
	 * @param {number} x
	 * @param {number} y
	 * @param {number} spriteSize
	 * @param {boolean} isFocusTarget
	 */
	function drawHudTargetingBracket(ctx, x, y, spriteSize, isFocusTarget) {
		if (!ctx.beginPath || !ctx.stroke) return;
		ctx.save();
		ctx.strokeStyle = isFocusTarget ? "#f59e0b" : "#fbbf24";
		ctx.lineWidth = isFocusTarget ? 3.5 : 2.5;
		ctx.shadowColor = isFocusTarget
			? "rgba(245, 158, 11, 0.95)"
			: "rgba(251, 191, 36, 0.85)";
		ctx.shadowBlur = isFocusTarget ? 14 : 10;
		const pad = isFocusTarget ? 10 : 8;
	 ...
	 * @returns {any|null}
	 */
	function findEnemyHitAtCoords(coords, bounds) {
		if (!coords || !Array.isArray(bounds)) return null;
		const px = coords.cssX !== undefined ? coords.cssX : coords.x;
		const py = coords.cssY !== undefined ? coords.cssY : coords.y;
		let closest = null;
		let minD = Infinity;
		for (const b of bounds) {
			...
			const radius = Math.max(50, b.w / 2 + 20);
			if (d <= radius && d < minD) {
				minD = d;
				closest = b;
			}
	...
	function renderQ2ClashCanvas(q2Clash, state) {
		lastQ2Clash = q2Clash;
		const canvas = /** @type {HTMLCanvasElement|null} */ (
			getElement("combat-backdrop-canvas")
		);
		if (!canvas) return;
		...
		const drawEnemyBattler = (enemy, idx, zScale, yPosBase, count, slotIdx) => {
			...
			const isBoss = Boolean(
				enemy.isBoss ||
				(enemy.key && String(enemy.key).toUpperCase().includes("BOSS")),
			);
			const targetEnemyIdx = resolveContextualEnemyIndex();
			...
			// Highlight pulse scaling & breathing elevation
			const highlightPulse = isHighlighted
				? 1.25 + Math.sin(Date.now() / 200) * 0.08
				: 1.0;
			const effectiveScale = (isBoss ? zScale * 1.35 : zScale) * highlightPulse;
			const spriteSize = 96 * effectiveScale;
			const drawY = isHighlighted ? yPosBase - 8 : yPosBase;

			if (enemy.alive) {
				renderedEnemyBounds.push({
					x: x - spriteSize / 2,
					y: drawY - spriteSize / 2,
					w: spriteSize,
					h: spriteSize,
					enemy,
					index: idx,
			...
			drawBattlerContactShadow(ctx, x, drawY, spriteSize);
			...
			drawFloatingBattlerHud(ctx, enemy, {
				x,
				y: drawY,
				spriteSize,
				effectiveScale,
		...
		backEnemies.forEach(({ e, idx }, slotIdx) => {
			drawEnemyBattler(e, idx, 1.6, H * 0.4, backEnemies.length, slotIdx);
		});

		// Draw Front Row Second - 2.2x Scale
		frontEnemies.forEach(({ e, idx }, slotIdx) => {
			drawEnemyBattler(e, idx, 2.2, H * 0.68, frontEnemies.length, slotIdx);
		});

		// Bind Pointer Events on Q2 Backdrop Canvas Idempotently
		if (!q2EventsBound) {
			q2EventsBound = true;

			canvas.addEventListener("pointermove", (ev) => {
				const coords = getNormalizedCanvasCoords(canvas, ev);
				const hit = findEnemyHitAtCoords(coords, renderedEnemyBounds);
				if (hit) {
					setEphemeralHover({
						id: hit.enemy.id,
						type: "ENEMY",
	...
	function renderQ3ChronicleStream(streamEl, logEntries = []) {
		...
		if (!Array.isArray(logEntries) || logEntries.length === 0) {
			streamEl.innerHTML =
				'<div class="chronicle-empty-hint">⚔️ Battle commenced. Awaiting tactical execution...</div>';
			return;
		}

		streamEl.innerHTML = logEntries
			.map((log) => {
				const text = typeof log === "string" ? log : (log?.msg || log?.text || String(log));
				const tone = typeof log === "object" && log?.type ? log.type : "";
				let typeClass = "dmg";
				let badgeLabel = "DMG";
				if (tone === "heal" || /heal|recover|restore/i.test(text)) {
					typeClass = "heal";
					badgeLabel = "HEAL";
				} else if (tone === "ember" || /ember|phoenix|aether|revive|relic/i.test(text)) {
					typeClass = "ember";
					badgeLabel = "EMBER";
				} else if (tone === "status" || /stun|poison|burn|ailment|paralyze/i.test(text)) {
					typeClass = "status";
					badgeLabel = "STATUS";
				} else if (tone === "guard" || /guard|brace|block/i.test(text)) {
					typeClass = "status";
					badgeLabel = "GUARD";
				} else if (/crit|weakness/i.test(text)) {
					typeClass = "crit";
					badgeLabel = "CRIT";
				} else if (tone === "system" || /ambush|victory|fallen|battle/i.test(text)) {
					typeClass = "status";
					badgeLabel = "LOG";
				}
				return `<div class="chronicle-entry ${typeClass}"><span class="chronicle-badge">${badgeLabel}</span> <span>${text}</span></div>`;
			})
			.join("");
	 ...
	 * @returns {void}
	 */
	function renderQ3ThreatOracleDeck(q3Oracle, state) {
		...
		if (intentFeedEl && q3Oracle?.threatVectors) {
			if (q3Oracle.threatVectors.length === 0) {
				...
				intentFeedEl
					.querySelectorAll(".intent-beacon-item")
					.forEach((beacon) => {
						/** @type {HTMLElement} */
						const htmlBeacon = /** @type {HTMLElement} */ (beacon);
						...
						htmlBeacon.addEventListener("mouseenter", () => {
							if (enemyId)
								setEphemeralHover({ id: enemyId, type: "ENEMY", index: 0 });
						});
		...
		if (affinityContainer && q3Oracle?.enemies) {
			...
			if (livingFoes.length > 0) {
				...
				(activeFoe.weaknesses || []).forEach((w) => {
					badges.push(
						`<span class="affinity-badge weak">WEAK: ${w} (1.5x)</span>`,
					);
				});
				(activeFoe.resistances || []).forEach((r) => {
					badges.push(
						`<span class="affinity-badge resist">RESIST: ${r} (0.5x)</span>`,
					);
				});
				(activeFoe.immunities || []).forEach((i) => {
					badges.push(
						`<span class="affinity-badge immune">IMMUNE: ${i} (0.0x)</span>`,
					);
				});
		...
		const chronicleStreamEl = getElement("combat-chronicle-stream");
		if (chronicleStreamEl) {
			const logs =
				state?.log || q3Oracle?.log || lastCombatState?.log || [];
			renderQ3ChronicleStream(chronicleStreamEl, logs);
		}
	}

	/**
	 * Builds Q4 radial chassis descriptor for the active hero turn.
				...
				onHover: () => {
					if (activeChar && defaultEnemy) {
						renderBasicTelemetry(
							activeChar,
							defaultEnemy,
							getElement("telemetry-dmg-range"),
							getElement("telemetry-hit-rate"),
	...
	/**
	 * Calculates vitals metrics and arc offsets for hero pedestal.
	 * @param {any} hero
	 * @returns {{ curHp: number, maxHp: number, curMp: number, maxMp: number, hpPct: number, mpPct: number, isCritical: boolean, hpOffset: number, mpOffset: number }}
	 */
	function resolveHeroPedestalStats(hero) {
		...
		const hpOffset = Math.max(0, Math.min(150, 150 * (1 - hpPct / 100)));
		const mpOffset = Math.max(0, Math.min(125, 125 * (1 - mpPct / 100)));
		return {
			curHp,
			maxHp,
			curMp,
	...
	/**
	 * Builds HTML template for hero arched vitals pedestal.
	 * @param {any} hero
	 * @param {ReturnType<typeof resolveHeroPedestalStats>} stats
	 * @param {string|null} avatarUrl
	 * @returns {string}
	 */
	function buildHeroPedestalTemplate(hero, stats, avatarUrl) {
		const { curHp, maxHp, curMp, maxMp, isCritical, hpOffset, mpOffset } =
			stats;
		return `
			<div class="hero-arched-vitals-container">
				<svg class="hero-arched-vitals-svg" viewBox="0 0 100 75" aria-hidden="true">
					<path class="vital-arc-track" d="M 10 68 A 40 54 0 0 1 90 68" />
					<path class="vital-arc-track mp" d="M 18 68 A 32 46 0 0 1 82 68" />
					<path class="vital-arc-hp ${isCritical ? "critical" : ""}" d="M 10 68 A 40 54 0 0 1 90 68" style="stroke-dashoffset: ${hpOffset}px;" />
					<path class="vital-arc-mp" d="M 18 68 A 32 46 0 0 1 82 68" style="stroke-dashoffset: ${mpOffset}px;" />
				</svg>
	...
	/**
	 * Binds hover, selection, and contextual radial events to hero pedestal stage.
	 * @param {HTMLElement} stage
	 * @param {any} hero
	 * @param {number} idx
	 * @param {CombatState} state
	 */
	function bindHeroPedestalEvents(stage, hero, idx, state) {
		stage.addEventListener("mouseenter", () =>
			setEphemeralHover({ id: hero.id, type: "HERO", index: idx }),
		);
		stage.addEventListener("mouseleave", () => setEphemeralHover(null));
	...
	/**
	 * Renders a single Hero Pedestal Stage in Quadrant 4.
	 * @param {any} hero Hero vitals node.
	 * @param {number} idx Hero slot index.
	 * @param {boolean} isFlareActive Whether flare linking is active.
	 * @param {CombatState} state Combat state.
	 * @returns {HTMLElement}
	 */
	function renderHeroPedestalStage(hero, idx, isFlareActive, state) {
		...
		const stats = resolveHeroPedestalStats(hero);
		const avatarUrl =
			resolvePartyBattlerUrl(hero) ||
			(typeof EmberlightPartyIcons !== "undefined" &&
				typeof EmberlightPartyIcons.get === "function"
				? EmberlightPartyIcons.get(hero.phenotype)
				: null) ||
			(typeof EmberlightIcons !== "undefined" &&
				typeof EmberlightIcons.get === "function"
				? EmberlightIcons.get(hero.phenotype)
				: null);

		stage.innerHTML = buildHeroPedestalTemplate(hero, stats, avatarUrl);
		bindHeroPedestalEvents(stage, hero, idx, state);
		return stage;
	}
	...
	return {
		...
		render(state, dispatch) {
			...
			if (
				...
				typeof EmberlightCombatBackdrop.render === "function"
			) {
			...
			const targetEnemyIdx = resolveContextualEnemyIndex();
			const targetEnemy = state.enemies?.[targetEnemyIdx] || null;
			const targetEnemyId =
				ephemeralHover?.type === "ENEMY"
					? ephemeralHover.id
					: targetEnemy?.id || null;
			...
			if (
				typeof EmberlightThreatOracle !== "undefined" &&
				typeof EmberlightThreatOracle.render === "function"
			) {
				EmberlightThreatOracle.render(
					"combat-ctb-ribbon-bar",
					{
						...
						enemies: state.enemies || [],
						forecastQueue: state.forecastQueue || null,
						activeHeroId: activeChar?.id || null,
						targetEnemyId,
						bossIntent:
							(state.enemies || []).find((e) => e.isBoss && e.alive) || null,
						selectedAction: state.pendingSkill || {
							type: state.selectedTab || "ATTACK",
						},
					},
					emit,
				);
